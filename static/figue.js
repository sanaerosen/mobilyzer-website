...
//				if (l.length != len) alert(l.length + "     -     " + len +"     -    " + l);
	...
	function agglomerate (labels, vectors, distance, linkage) {
		...
		for (p = 0 ; p < N-1 ; p++) {
			...
			//Ã‚ overwriteÃ‚ rowÃ‚ c1Ã‚ with respect to the linkage type
			for (j = 0 ; j < N ; j++) {
			...
			//Ã‚ infinity Ã‚Â­outÃ‚ oldÃ‚ rowÃ‚ c2Ã‚ andÃ‚ columnÃ‚ c2
			for (i = 0 ; i < N ; i++)
				distMatrix[i][c2] = distMatrix[c2][i] = Infinity ;
	
			//Ã‚ updateÃ‚ dminÃ‚ andÃ‚ replaceÃ‚ onesÃ‚ thatÃ‚ previousÃ‚ pointedÃ‚ to c2 to point to c1
			for (j = 0; j < N ; j++) {
				if (dMin[j] == c2)
					dMin[j] = c1;
